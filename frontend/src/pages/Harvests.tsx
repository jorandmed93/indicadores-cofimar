...
const Harvests: React.FC = () => {
  ...
  const fetchHarvests = () => {
    ...
    client.get('/harvests', { params })
      .then(res => {
        ...
        // Calculate QC averages on the page
        let diffSum = 0;
        let maxDiff = 0;
  ...
  useEffect(() => {
    fetchHarvests();
  }, [page, limit, selectedActivity, selectedSector]);
  ...
  return (
    <div className="p-8 space-y-7">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-5 rounded-2xl border-l-4 border-l-cofimar-primary flex items-center space-x-4 shadow-xl">
          ...
          <div>
            <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider block">Merma Promedio (Libras)</span>
            <h4 className="text-2xl font-display font-bold text-white mt-1 font-mono">
              {loading ? '...' : `${Math.round(avgDiffLbs).toLocaleString()} lbs`}
            </h4>
          </div>
        </div>

        <div className="glass-card p-5 rounded-2xl border-l-4 border-l-cofimar-danger flex items-center space-x-4 shadow-xl">
          <div className="w-10 h-10 bg-cofimar-danger/10 rounded-xl flex items-center justify-center border border-cofimar-danger/20">
            <AlertTriangle className="w-5 h-5 text-cofimar-danger" />
          </div>
          <div>
            <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider block">Merma Máxima Registrada</span>
            <h4 className="text-2xl font-display font-bold text-white mt-1 font-mono">
        ...
        <div className="glass-card p-5 rounded-2xl border-l-4 border-l-cofimar-success flex items-center space-x-4 shadow-xl">
          <div className="w-10 h-10 bg-cofimar-success/10 rounded-xl flex items-center justify-center border border-cofimar-success/20">
            <CheckCircle className="w-5 h-5 text-cofimar-success" />
          </div>
          <div>
            <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider block">Total Transacciones</span>
            <h4 className="text-2xl font-display font-bold text-white mt-1 font-mono">
      ...
      {loading ? (
        <div className="h-[400px] flex flex-col items-center justify-center space-y-4">
          ...
          <span className="text-slate-400 font-mono text-xs">Cargando transacciones de báscula...</span>
        </div>
      ) : (
        <div className="glass-card rounded-2xl shadow-xl overflow-hidden border border-cofimar-border/50">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs whitespace-nowrap">
              ...
              <tbody className="divide-y divide-cofimar-border/30">
                {harvests.length === 0 ? (
                  <tr>
                    <td colSpan={11} className="py-10 text-center font-mono text-slate-500">
                  ...
                  harvests.map((h, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-800/10 transition">
                        ...
                        <td className="py-3.5 px-5 font-mono">{h.harvest_date}</td>
                        <td className="py-3.5 px-5 text-slate-400">{h.sector}</td>
                        <td className="py-3.5 px-5 text-right font-mono">{Math.round(h.lbs_farm || 0).toLocaleString()}</td>
                        <td className="py-3.5 px-5 text-right font-mono">{Math.round(h.lbs_plant || 0).toLocaleString()}</td>
                        ...
                        <td className="py-3.5 px-5 text-right font-mono">{parseFloat(h.gr_farm || 0).toFixed(2)}</td>
                        <td className="py-3.5 px-5 text-right font-mono">{parseFloat(h.gr_plant || 0).toFixed(2)}</td>
                        ...
                        <td className="py-3.5 px-5 text-slate-400">{h.sector_chief}</td>
                      </tr>
                    );
                  })
                )}
          ...
          <div className="bg-slate-850/50 border-t border-cofimar-border/60 py-4 px-6 flex flex-col md:flex-row md:items-center justify-between gap-4 font-mono text-xs">
            <span className="text-slate-400">
              Mostrando <span className="text-white font-bold">{harvests.length}</span> de <span className="text-white font-bold">{total}</span> transacciones
            </span>
